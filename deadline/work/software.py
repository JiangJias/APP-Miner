...
class SOFTWARE(App):
    ...
    def __init__(self, app, cmd):
        super(SOFTWARE, self).__init__(app, cmd, BUILDER, None)
    '''
        toks = self.tag.split(".")
        if int(toks[0]) < 4:
    ...
    def build_impl(self, psrc, pobj, pbin, pext, plog):
        with cd(pobj):
            #cmd = "make -j80 vmlinux modules"
            with open(plog, "w") as f:
                if shell(self.cmd + ' V=1 -j' + str(int(cpu_count() / 2)), out = f) == 0:
                    LOG_INF("Build done")
                else:
